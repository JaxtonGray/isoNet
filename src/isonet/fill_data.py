...
def get_runs_gdf(start_date: str, end_date: str, coords: iter, crs: str = 'EPSG:4326',
                 isDaily: bool = False, freq: str = 'MS') -> pd.DataFrame:
    # Function to get a dataframe with all the combinations of unique coordinates and dates within the specified range
    # Args:
    #   - start_date: The start date as a string in the format 'YYYY-MM-DD'
    #   - end_date: The end date as a string in the format 'YYYY-MM-DD'
    #   - coords: An iterable of unique coordinates (e.g., a list of shapely Point objects, geopandas GeoSeries)
    #   - isDaily: A boolean indicating whether to use daily frequency (True) or monthly frequency (False), default is False
    #   - crs: The coordinate reference system for the GeoDataFrame, default is 'EPSG:4326'
    #   - freq: The frequency of the date range, default is 'MS' (month start)
    # Returns:
    #   - A pandas DataFrame containing the range of dates
    if not isDaily:
